...
AuthLayout
  .text-center.mb-6
    ...
    p.text-sm.text-muted-foreground.mt-1 Informe seu e-mail para receber as instruções de recuperação
  ...
  template(v-else)
    Alert.border-success(class="bg-success/10")
      CheckCircle.h-4.w-4.text-success
      AlertDescription.text-success
        | Enviamos um e-mail para
        strong  {{ email }}
        |  com as instruções para redefinir sua senha. Verifique sua caixa de entrada.

  .mt-6.text-center
    RouterLink.inline-flex.items-center.gap-1.text-sm.text-primary(to="/login" class="hover:underline")
      ArrowLeft.h-4.w-4
      | Voltar para o login
</template>
