...
header.sticky.top-0.z-50.w-full.border-b.border-border.backdrop-blur(class="bg-background/95 supports-[backdrop-filter]:bg-background/60")
  .container.mx-auto.flex.h-16.items-center.justify-between.px-4
    ...
    nav.hidden.items-center.gap-6(class="md:flex")
      a.text-sm.font-medium.text-muted-foreground.transition-colors.cursor-pointer(class="hover:text-foreground"
    ...
    .flex.items-center.gap-3
      button.rounded-md.p-2.text-muted-foreground.transition-colors(class="hover:text-foreground"
        @click="theme.toggle()"
  ...
  .border-b.border-border.bg-background(v-if="mobileMenuOpen" class="md:hidden")
    nav.flex.flex-col.gap-2.p-4
      a.text-sm.font-medium.text-muted-foreground.py-2.cursor-pointer(
        v-for="link in navLinks"
        :key="link.href"
        @click="scrollToSection(link.href)"
      ) {{ link.label }}
